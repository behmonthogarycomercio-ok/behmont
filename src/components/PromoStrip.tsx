...
export default function PromoStrip({ promotions }: { promotions: Promotion[] }) {
  ...
  return (
    <section className="bg-plate-100 py-10">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 grid gap-4 sm:grid-cols-2">
        {promotions.map((promo) => (
          <Link
            key={promo.id}
            href={promo.cta_link || '#'}
            className="group relative overflow-hidden rounded-xl2 bg-white min-h-[180px] flex items-center px-6 shadow-card"
          >
            {promo.image_url && (
              <Image
                src={promo.image_url}
